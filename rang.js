...
//*********************************************************
function range_go(crit,k,m){
    let n,w2,z,knum,i,nn,pw=[],wrange=[];

//*******************************************
     nn=permrepeat(crit,k,m,wrange,pw);

//********************************************  
    document.write(crit+" Variants:");
    document.write(num);
    document.write("<br>");
    ...
    document.write("<br>");
    for (i = 0; i < nn; i++) {
        document.write(i+1+": "+wrange[i].toFixed(znaki)+"  "+pw[i].toFixed(znaki));
        document.write("<br>");  
    }
...
function data_ansari(m,n,astat,pw) {
...
//*************************************
function wilcoxon(m,n,range,pw) {
...
*/
 ...
 work[1]=0;inx=maxmn;
  ...
  //w[i] - Frequencies
    ...
    for(i=1;i<=mn;i++) pw[i]=pw[i]/sum;

    return(mn);
 }
//************ Критерий серий точное распределение********
function seriestest(k,ksr,n,m,w,p,zx) {
   let s1,m1,m2;

   m1=m[0];m2=m[1];n=0;
   for (i=1;i<=k;i++)  n=n+m[i];
   w2=cnm(n,m1);
   s=0;
  for (i=1;i<=5000;i++) {
    kcur=1+i;pc=0;
   ...
   s+=pc;w[i]=parseFloat(pc);
   p[i]=parseFloat(s/w2);
   zx[i]=parseFloat(kcur);
   if(p[i]>=1) break;
   }
  ksr=i;
   return ksr;
